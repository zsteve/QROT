...
ε = 1.0
π = sparse(quadreg(μ, C / Cmean, ε, OptimalTransport.SymmetricQuadraticOTNewton(); maxiter = 100))

using BenchmarkTools
@benchmark quadreg($μ, $(C / Cmean), ε, OptimalTransport.SymmetricQuadraticOTNewton(); maxiter = 100)
@info "sparsity: $(nnz(π) / N^2)"
...
@info "sparsity: $(nnz(S) / N^2)"

ε = 1.0
alg = OptimalTransport.SymmetricQuadraticOTNewtonAS(S);
γ = quadreg(μ, C / Cmean, ε, alg; maxiter = 100);
dropzeros!(γ);
@info "sparsity: $(nnz(γ) / N^2)"

@benchmark quadreg($μ, $(C / Cmean), ε, $alg; maxiter = 100)

# check bistochasticity
norm(sum(γ; dims = 2) .- 1, Inf)
norm(sum(γ; dims = 1) .- 1, Inf)
# check diagonal
norm(diag(γ), Inf)
...
using PythonOT
@benchmark PythonOT.Smooth.smooth_ot_dual($μ, $μ, $(C / Cmean), ε)

π_pot = PythonOT.Smooth.smooth_ot_dual(μ, μ, C / Cmean, ε)
